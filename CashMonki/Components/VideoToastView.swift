//
//  VideoToastView.swift
//  CashMonki
//
//  Toast with MP4 video animations
//

import SwiftUI

struct VideoToastView: View {
    let message: String
    let type: ToastType
    ...
    enum ToastType {
        ...
        var videoName: String {
            ...
            case .done:
                return "toast-done"
            case .error:
                return "error-animation" // You can add this MP4 later
            }
        }
        ...
        var shouldLoop: Bool {
            switch self {
            case .scanning:
                return true  // Keep looping while analyzing
            case .done:
                return false // Play once when done
            case .error:
                return false // Play once for error
            }
        }
    }
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 0) {
            // Receipt icon background with video animation
            ZStack {
...
// MARK: - Video Toast Manager

class VideoToastManager: ObservableObject {
    @Published var currentToast: ToastData?
    
    struct ToastData: Identifiable {
        let id = UUID()
        var message: String
        var type: VideoToastView.ToastType
        var isShowing: Bool = true
    }
    
    func show(_ message: String, type: VideoToastView.ToastType) {
...
// MARK: - Video Toast Overlay Modifier

struct VideoToastOverlay: ViewModifier {
    @StateObject private var videoToastManager = VideoToastManager()
    
    func body(content: Content) -> some View {
        content
            .overlay(alignment: .bottom) {
                if let toast = videoToastManager.currentToast {
                    VideoToastView(
                        message: toast.message,
                        type: toast.type,
                        isShowing: Binding(
                            get: { toast.isShowing },
                            set: { _ in videoToastManager.dismiss() }
                        )
                    )
            ...
            .environmentObject(videoToastManager)
    }
}

extension View {
    func withVideoToast() -> some View {
        modifier(VideoToastOverlay())
    }
}
