...
extension HomePage {
    ...
    func analyzeReceiptImage(_ image: UIImage, source: AnalyzingSource = .scan) {
        // Debug: Check if toast manager is available
        print("🔍 DEBUG: Starting receipt analysis, toast manager available: \(toastManager)")
        
        // Start the toast animation
        toastManager.startReceiptAnalysis()
        print("🍞 DEBUG: Toast started")
        ...
        AIReceiptAnalyzer.shared.analyzeReceipt(image: image) { result in
            DispatchQueue.main.async {
                ...
                case .failure(let error):
                    ...
                    // Dismiss scanning toast and show failed message with failed animation
