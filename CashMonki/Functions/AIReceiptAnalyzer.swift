...
class AIReceiptAnalyzer {
    ...
    static func parseReceiptDate(_ dateString: String) -> Date? {
        ...
        for (formatter, description) in formatters {
            if let parsedDate = formatter.date(from: trimmed) {
                ...
                // If it's a date-only format (no time component), set to 12:00 PM for visibility
                if description.contains("date only") {
                    let calendar = Calendar.current
                    let components = calendar.dateComponents([.year, .month, .day], from: parsedDate)
                    var newComponents = components
                    newComponents.hour = 12 // Set to noon instead of midnight for better UX
    ...
    func analyzeReceipt(image: UIImage, completion: @escaping (Result<ReceiptAnalysis, Error>) -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                let result = try self.parseOpenRouterResponse(data)
    ...
    private func parseOpenRouterResponse(_ data: Data) throws -> ReceiptAnalysis {
        ...
        let date = Self.parseReceiptDate(dateString) ?? Date()
