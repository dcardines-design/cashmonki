...
class SafetyMonitor {
    ...
    private func checkMemoryUsage() {
        ...
        if result == KERN_SUCCESS {
            ...
            if memoryUsage > 500 { // Alert if over 500MB
                let alert = SafetyAlert(
                    severity: .warning,
    ...
    private func checkDiskSpace() {
        let freeSpace = getAvailableStorageSpaceForMonitoring()
        if freeSpace < minimumFreeSpaceThresholdForMonitoring {
            let alert = SafetyAlert(
                severity: .error,
                category: .storage,
                message: "Critically low disk space",
                details: "Available: \(freeSpace / 1024 / 1024)MB"
            )
            alertHandler(alert)
        }
    }
    ...
    private func checkSystemPerformance() {
        ...
        if processInfo.thermalState == .critical {
            let alert = SafetyAlert(
                severity: .critical,
                category: .performanceIssue,
                message: "Device thermal throttling detected",
                details: "System is overheating, migration may be unreliable"
            )
            alertHandler(alert)
        }
    }
    
    // MARK: - Helper methods for monitoring
    
    private func getAvailableStorageSpaceForMonitoring() -> Int {
        // Get available storage space for monitoring
        let documentsDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        
        do {
            let resourceValues = try documentsDir.resourceValues(forKeys: [URLResourceKey.volumeAvailableCapacityForImportantUsageKey])
            return Int(resourceValues.volumeAvailableCapacityForImportantUsage ?? 0)
        } catch {
            print("⚠️ Could not determine available storage space: \(error)")
            return 0
    ...
    private var minimumFreeSpaceThresholdForMonitoring: Int {
        // Return minimum free space threshold for monitoring
